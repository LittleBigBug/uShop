...
public class EssentialsWorthConverter {
	...
	public synchronized static void convert(Main plugin, CommandSender cs) {
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () -> {
            if(!Bukkit.getPluginManager().isPluginEnabled("Essentials")){
                cs.sendMessage("§cEssentials not installed!");
                return;
            }
            ...
            if(!worthFile.exists()) {
                cs.sendMessage("§cWorth file does not exist! (" + worthFile.getAbsolutePath() + ")");
                return;
            }
            ...
            Map<String, Material> essentialsRenaming = Arrays.stream(Material.values()).
                    collect(Collectors.toMap(m -> m.name().replace("_", "").toLowerCase(), m -> m));

            Map<String, Material> essentialsLegacyRenaming = Arrays.stream(legacyNames).
            ...
            for (String itemName : worthcfg.getConfigurationSection("worth").getKeys(false)) {
                ...
                Material mat = essentialsRenaming.getOrDefault(itemName, null);

                if (mat == null)
                    mat = essentialsLegacyRenaming.getOrDefault(itemName, null);
